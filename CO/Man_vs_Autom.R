...
input = read.csv("/Users/r.nacheva/Downloads/Manual_Automated_No_CancellationCompensations_2022_10_05.csv")
...
segmentation <-  list(segment1 = c('total')
                    #  ,segment2 = c('weeks_num_with_ride')
...
kpi_name = c( 'Utilisation','AccR','PaxCanc_over_acc','5min_PaxCanc_over_acc')  # same name as inserted in the balancing
kpi_nominator = c('Active_hours','acc','pax_can',"pax_can_above_5min") # same as in data set
kpi_denominator = c('Supply_hours','broadcast','acc_won','acc_won') # same as in data set
...
kpis <-c('Utilisation','AccR','PaxCanc_over_acc',"Supply_hours","rides","revenue","driver_earning","pax_can","pax_can_above_5min","acc",
         "acc_won","ignored","broadcast"    
)

# Define column name where you stored the assigment (treatment or control)
assignment = 'driver_group'

# Define the name of your Treatment and Control group
#Treatment_name = 'enabled_manual' #  'tired_from8'
Treatment_name = 'enabled' #  'tired_from8'
#Treatment_name = 'tired_from8' #  'tired_from8'
Control_name = 'disabled'

# Set up you confidence interval (95% as a standard recommendation)
conf_int = 0.95

# Define Output file name
outputname <- '/Users/r.nacheva/Downloads/PaxCancellationsCompensations_automated.csv'
