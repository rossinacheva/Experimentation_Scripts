...
input = read.csv("/Users/r.nacheva/Downloads/3447420_2022_10_11.csv")
...
segmentation <-  list(segment1 = c('total')
                      ,segment2 = c('market')
                      ,segment3 = c('market','ltv_segment')
                    #  ,segment4 = c('market','is_b2b') #b2b has no utilisation_weighted
...
input$active_hours = input$utilisation*input$supply_hours

# Create ratio KPIs: 

kpi_name = c( 'Utilisation_weighted','AccR','RejR','IngoreR',"PassCanc_perAccepted","time_to_accept_calc","time_to_reject_calc")  # same name as inserted in the balancing
kpi_nominator = c('active_hours','accepted','rejected','ignored','pax_can',"time_to_accept_sum","time_to_reject_sum") # same as in data set
kpi_denominator = c('supply_hours','broadcasts','broadcasts','broadcasts','accepted','accepted','rejected') # same as in data set
...
kpis <-c('Utilisation_weighted','AccR','RejR','IngoreR',"PassCanc_perAccepted","driver_earning","supply_hours","rides","time_to_accept_calc","time_to_reject_calc"  
        ,"accepted" ,"rejected" , "ignored" ,"broadcasts" , "driver_aborts" ,"pax_can" ,"rides_surge"
)

# Define column name where you stored the assigment (treatment or control)
assignment = 'exp_group_name'

# Define the name of your Treatment and Control group
Treatment_name = 'test' #  'tired_from8'
...
outputname <- '/Users/r.nacheva/Downloads/Driver_Screen.csv'
