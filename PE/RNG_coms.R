...
input_b = read.csv("/Users/r.nacheva/Downloads/ZR & Churned target - Redash_Upload.csv") # target
input_a = read.csv("/Users/r.nacheva/Downloads/RNG_comms_2022_10_27.csv") # redash

input = merge(input_b, input_a, by = 'id_driver', all.x = TRUE)

input$activated = ifelse(input$rides_after > 0, 1,0)
...
kpi_name = c( 'EtRC')  # same name as inserted in the balancing
kpi_nominator = c('revenue') # same as in data set
kpi_denominator = c('revenue') # same as in data set
...
kpis <-c('rides_after','bf_revenue','drv_earnings','activated'
)
...
Treatment_name = 'Automated_C' #  'tired_from8'
...
outputname <- '/Users/r.nacheva/Downloads/RNG_comms_Automated_C.csv'
...
#with_rides = subset(input, input$rides_after >0)
#write.table(with_rides,"/Users/r.nacheva/Downloads/RNG_comms_rawdata.csv", sep=',', dec='.',row.names=F)
#write.table(input,"/Users/r.nacheva/Downloads/RNG_comms_rawdata_all.csv", sep=',', dec='.',row.names=F)
