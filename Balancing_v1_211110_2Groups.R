...
input<- read.csv("/Users/r.nacheva/Downloads/Radius_Test_Nov_2021_11_24.csv")

summary(input)

names(input)

input= subset(input, input$Supply_hours > 0)

head(input)
# Define segmentation : city, service, total
input$segment_start = substr(input$starting_segment, 1, 6)


# Define segmentation : COUNTRY, SEASON, Total
segmentation <-  list(segment1 = c('segment_start','period')
)
# Define your entity
entity <- 'id_driver'

# Define new KPIs
input$accR = input$acc/input$broadcast
input$util = input$Active_hours / input$Supply_hours

# Main KPIs you want to balance on and take into concideration for outlier detection
balancing <- c('Supply_hours','active_days_period','driver_earning','Rides','Revenue','acc','accR','util')

# Buckets split for treatment - put 0.5 for 50% treatment (and 50% control)
buckets = 0.5

# Parameter for allowed mean difference & outlier detection
mean_difference = 0.05
outlier_threshold = 0.999

# Define Output file name
outputname <- '/Users/r.nacheva/Downloads/Balancing_CL_Radius_Nov_211124.csv'
...
for (level in 1:length(segmentation)){
  ...
  for(i_comb in 1:combinations){
    ...
    {  
      ...
      score<-rbind(score_mean,score_pValue) 
      p = p+1
      if (sum(score > mean_difference) == 0| p ==1000) break
    }
