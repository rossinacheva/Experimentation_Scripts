...
input<- read.csv("/Users/r.nacheva/Downloads/Pass_Elasticity_2022_06_22.csv")

input = subset(input, input$activity_status_platform %in% c("ACTIVE",'INACTIVE') & 
                 input$ltv_agg %in% c('High','Med') )
summary(input)
...
segmentation <-  list(segment1 = c('activity_status_platform','ltv_agg')
)
...
kpi_name = c('accR')  # same name as inserted in the balancing
kpi_nominator = c('rides') # same as in data set
kpi_denominator = c('rides') # same as in data set

# Main KPIs you want to balance on and take into consideration for outlier detection
# Start with your main KPI
balancing <- c('rides',"ratio_before","ratio_after","eyeballs_before","eyeballs_after","searches_before","searches_after","no_search_fare_before_avg"
               ,"search_fare_before_avg","no_search_fare_after_avg","search_fare_after_avg"        
               ,"GMV","revenue")
...
mean_difference = 0.03 # max allowed difference between groups (even when insignificant results)
outlier_threshold = 0.99 # 1 means no outliere detection - all 1 (100%) of the data is used

# Define Output file name
outputname <- '/Users/r.nacheva/Downloads/Balancing_PE_PriceSensitive_Passengers_220622_v2.csv'
