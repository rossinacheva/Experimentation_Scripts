...
input_a<- read.csv("/Users/r.nacheva/Downloads/Radius_Hourly_2022_06_15 (1).csv")
input_b<- read.csv("/Users/r.nacheva/Downloads/Radius_Hourly_2022_06_15.csv")


input = rbind(input_a,input_b)

input = subset(input, input$service %in% c("Beat",'Lite'))
...
input$week_day <- weekdays(as.Date(input$hour))
input$hour_num = strftime(input$hour, format = "%H")
input$week_day = ifelse (input$week_day %in% c('Saturday','Sunday'),'weekend','weekday')



input$time_day = ifelse(input$hour_num %in% c(23,0,1,2,3,4) & input$week_day == 'weekday', 'wd_night_increase'
                      , ifelse(input$hour_num %in% c(6,7,8,9) & input$week_day == 'weekday', 'wd_morning_decrease'
                      , ifelse(input$hour_num %in% c(18) & input$week_day == 'weekday', 'wd_18_decrease'         
                      , ifelse(input$hour_num %in% c(5,6) & input$week_day == 'weekend', 'we_night_increase'
                      ,ifelse(input$hour_num %in% c(18) & input$week_day == 'weekend', 'we_18_decrease'
                     ,'no_radius_change')))))
...
segmentation <-  list(segment1 = c('total')
                      ...
                      ,segment3 = c('week_day')
                      ,segment4 = c('time_day')
                      ,segment5 = c('service','week_day')
                      ,segment6 = c('service','time_day')
)
...
kpis <-c('Supply_hours','Utilisation','driver_earning','pax_can','Rides','Revenue','acc','AccR','broadcast','acc_upto_2km','acc_upto_3km',
         'acc_above_3km','eta','eta_upto_2km','eta_upto_3km'
)
...
outputname <- '/Users/r.nacheva/Downloads/Results_Radius_pHour_BOG_220615_v2.csv'
