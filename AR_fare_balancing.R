...
input<- read.csv("/Users/r.nacheva/Downloads/Test_y_Control_-_Fare_adjustment_2022_05_05.csv")
...
# Define segmentation : LIke country, driver type - most granular combination level. If no segmentation, please put 'total'
segmentation <-  list(segment1 = c('frequency_current')
)
...
balancing <- c('avg_daily_rides','avg_daily_revenue','utilisation','supply_hours')


# Create ratio KPIs: 
kpi_name = c('utilisation')  # same name as inserted in the balancing
kpi_nominator = c('active_hours') # same as in data set
kpi_denominator = c('supply_hours') # same as in data set
...
outlier_threshold = 0.999 # 1 means no outliere detection - all 1 (100%) of the data is used

# Define Output file name
outputname <- '/Users/r.nacheva/Downloads/Balancing_AR_fare_miscalc.csv'
