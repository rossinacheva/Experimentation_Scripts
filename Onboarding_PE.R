...
input = read.csv("/Users/r.nacheva/Downloads/[PE]_New_onboarding_-_results_by_periods_RN_2022_10_07 (3).csv")
...
kpi_name = c( 'revenue_total')  # same name as inserted in the balancing
kpi_nominator = c('bf_revenue') # same as in data set
kpi_denominator = c('flag_activated') # same as in data set
...
kpis <-c("days_first_ride", "rides", "revenue" ,"bf_revenue" ,"drv_earnings","flag_activated_tot","running_rides", "running_revenue" ,"running_bf_revenue"  
        , "running_drv_earnings"
)
...
outputname <- '/Users/r.nacheva/Downloads/Onboarding_New_Drivers_PE_221007_v1.csv'
...
for (level in 1:length(segmentation)){
  ...
  for(i_comb in 1:combinations){
    ...
    Control$flag_activated_tot = ifelse(Control$days_first_ride >=1,1,0)
    Treatment$flag_activated_tot = ifelse(Treatment$days_first_ride >=1,1,0)
